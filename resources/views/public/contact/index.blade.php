    ...
    <section class="bg-[#fff8f8] py-20">
        <div class="mx-auto grid max-w-[1120px] gap-10 px-5 lg:grid-cols-[0.9fr_1.1fr] lg:px-0">
            <div>
                ...
                <h1 class="mt-4 font-heading text-[42px] font-black leading-tight text-etc-on-surface md:text-[56px]">Mulai konsultasi dengan ETC Planet</h1>
                <p class="mt-5 text-[16px] leading-8 text-etc-on-muted">Kirim pertanyaan awal tentang program, jadwal, biaya, atau placement test. Tim ETC akan menghubungi kamu kembali.</p>
                ...
                <div class="mt-8 space-y-4">
                    @foreach ([
                        ...
                        <div class="flex gap-4 rounded-2xl border border-[#eeb8c9] bg-white p-5 shadow-soft">
                            ...
                            <div>
                                <p class="font-heading text-sm font-black text-etc-on-surface">{{ $info['label'] }}</p>
            ...
            <div class="rounded-[24px] border border-[#eeb8c9] bg-white p-6 shadow-panel md:p-8">
                ...
                <form method="POST" action="{{ route('public.contact.store') }}" class="space-y-5">
                    @csrf
                    <div>
                        <label for="name" class="font-heading text-sm font-bold text-etc-on-surface">Nama Lengkap</label>
                    ...
                    <div class="grid gap-5 md:grid-cols-2">
                        ...
                        <div>
                            <label for="phone" class="font-heading text-sm font-bold text-etc-on-surface">No WhatsApp</label>
                    ...
                    <div>
                        <label for="subject" class="font-heading text-sm font-bold text-etc-on-surface">Subjek</label>
                        <input id="subject" name="subject" value="{{ old('subject', $defaultSubject) }}" class="mt-2 w-full rounded-lg border border-[#e2bdc7] px-4 py-3 outline-none focus:border-etc-magenta focus:ring-4 focus:ring-etc-magenta/10">
                        @error('subject') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="message" class="font-heading text-sm font-bold text-etc-on-surface">Pesan</label>
                        <textarea id="message" name="message" rows="6" class="mt-2 w-full rounded-lg border border-[#e2bdc7] px-4 py-3 outline-none focus:border-etc-magenta focus:ring-4 focus:ring-etc-magenta/10" required>{{ old('message', $defaultMessage) }}</textarea>

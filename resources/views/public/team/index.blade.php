<x-layouts.public title="Team Pengajar">
    ...
    <section class="bg-[#fff8f8] py-20">
        <div class="mx-auto max-w-[1120px] px-5 text-center lg:px-0">
            ...
            <h1 class="mt-4 font-heading text-[42px] font-black leading-tight text-etc-on-surface md:text-[56px]">Pengajar Profesional ETC Planet</h1>
            <p class="mx-auto mt-5 max-w-2xl text-[16px] leading-8 text-etc-on-muted">Instructor yang tampil di halaman ini berasal dari data user role instructor dengan status aktif dan ditandai tampil di team page.</p>
        </div>
    </section>
                ...
                <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
                    @foreach ($instructors as $teacher)
                        <article class="rounded-[22px] border border-[#eeb8c9] bg-white p-6 text-center shadow-soft">
                            ...
                            <h2 class="mt-6 font-heading text-xl font-black text-etc-on-surface">{{ $teacher->name }}</h2>
                            ...
                            <p class="mt-5 text-sm leading-7 text-etc-on-muted">{{ $teacher->instructor_bio ?? 'Bio pengajar akan tampil setelah dilengkapi di data instructor.' }}</p>
                        </article>
                    @endforeach
                </div>
            @else
                <div class="rounded-[22px] border border-dashed border-[#eeb8c9] bg-white p-10 text-center shadow-soft">
                    ...
                    <h2 class="mt-4 font-heading text-2xl font-black text-etc-on-surface">Belum ada instructor yang dipublish</h2>
                    <p class="mt-3 text-etc-on-muted">Tambahkan user instructor aktif dengan show_on_team_page agar tampil di sini.</p>
                </div>
            @endif
        </div>
    </section>

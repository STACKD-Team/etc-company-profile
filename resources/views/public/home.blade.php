<x-layouts.public title="Beranda">
    ...
    <section class="relative overflow-hidden bg-[linear-gradient(108deg,#ffffff_0%,#ffffff_54%,#fdeaf0_54%,#fdeaf0_100%)]">
        <div class="mx-auto grid min-h-[760px] max-w-[1120px] items-center gap-12 px-5 py-20 lg:grid-cols-[1.02fr_0.98fr] lg:px-0">
            <div>
                ...
                <h1 class="mt-4 max-w-[560px] font-heading text-[44px] font-black leading-[1.18] tracking-normal text-etc-on-surface md:text-[56px] lg:text-[62px]">
                    Belajar Lebih Seru, Masa Depan Lebih Cerah
                </h1>
                <p class="mt-8 max-w-[520px] text-[17px] leading-8 text-etc-on-muted">
                ...
                <div class="mt-10 flex flex-col gap-4 sm:flex-row">
                    <a href="{{ route('registrations.programs.index') }}" class="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-etc-magenta px-8 py-3 font-heading text-[14px] font-bold text-white shadow-soft transition hover:bg-etc-primary">
                        Daftar Sekarang
                        <span class="material-symbols-outlined text-base">arrow_forward</span>
                    </a>
                    <a href="#program" class="inline-flex min-h-12 items-center justify-center rounded-full border-2 border-[#e7b8c8] bg-white/70 px-8 py-3 font-heading text-[14px] font-bold text-etc-on-surface transition hover:border-etc-magenta hover:text-etc-magenta">
                        Lihat Program
                    </a>
                </div>
                ...
                <div class="mt-7 flex flex-wrap gap-8 text-[13px] font-bold text-etc-on-surface">
            ...
            <div class="relative mx-auto w-full max-w-[470px] lg:mx-0">
                ...
                <div class="absolute -bottom-7 -left-4 flex items-center gap-4 rounded-2xl border border-[#f3c8d7] bg-white/95 px-5 py-4 shadow-[0_16px_28px_rgba(58,44,51,0.18)] backdrop-blur md:-left-7">
                    ...
                    <div class="leading-tight">
                        <p class="font-heading text-[14px] font-black text-etc-on-surface">Biaya Pendaftaran</p>
    ...
    <section class="bg-etc-charcoal py-12 text-white">
        <div class="mx-auto grid max-w-[1120px] grid-cols-2 gap-y-8 px-5 md:grid-cols-4 lg:px-0">
            @foreach ([
                ...
                <div @class(['text-center', 'md:border-l md:border-white/10' => $index > 0])>
                    ...
                    <p class="mt-4 font-heading text-[11px] font-bold uppercase tracking-[0.2em] text-white/60">{{ $stat['label'] }}</p>
                </div>
            @endforeach
        </div>
    </section>
    ...
    <section id="program" class="bg-[#fff7f7] py-24">
        <div class="mx-auto max-w-[1120px] px-5 lg:px-0">
            <div class="mx-auto max-w-[650px] text-center">
                <h2 class="font-heading text-[36px] font-black leading-tight text-etc-on-surface">Program Kursus Unggulan</h2>
                <p class="mt-4 text-[15px] leading-7 text-etc-on-muted">
                        ...
                        <article class="group overflow-hidden rounded-[22px] border border-[#eeb8c9] bg-white shadow-soft">
                            ...
                            <div class="p-6">
                                <div class="flex items-center justify-between gap-3">
                                    <h3 class="font-heading text-[25px] font-black leading-tight text-etc-on-surface">{{ $program->name }}</h3>
                                    <span class="inline-flex shrink-0 rounded-md bg-[#ffe6f3] px-3 py-2 font-heading text-[12px] font-black text-etc-on-surface">
                                        {{ str($program->target_age ?? 'all')->headline() }}
                                    </span>
                                </div>
                                <p class="mt-4 min-h-[72px] text-[15px] leading-7 text-etc-on-muted">{{ $program->description ?: 'Program aktif ETC Planet dengan kelas kecil dan pendampingan instruktur.' }}</p>
                                <div class="mt-7 border-t border-[#f0dde3] pt-5">
                                    <div class="flex items-end justify-between">
                                        <div>
                                            <p class="font-heading text-[11px] font-bold uppercase tracking-[0.18em] text-etc-on-muted">Mulai Dari</p>
                                            <p class="mt-2 font-heading text-[22px] font-black text-etc-magenta">{{ $formatMoney($program->price) }}</p>
                                        </div>
                                        <a href="{{ route('public.programs.show', $program) }}" class="text-etc-magenta" aria-label="Lihat detail {{ $program->name }}">
            ...
            @else
                <div class="mx-auto mt-14 max-w-2xl rounded-[22px] border border-dashed border-[#eeb8c9] bg-white p-8 text-center shadow-soft">
                    ...
                    <h3 class="mt-4 font-heading text-2xl font-black text-etc-on-surface">Data program sedang disiapkan</h3>
                    <p class="mt-3 text-etc-on-muted">Kamu tetap bisa konsultasi program yang cocok melalui form kontak ETC Planet.</p>
                </div>
            @endif
        </div>
    </section>
    ...
    <section class="bg-[#fff0f3] py-20">
        <div class="mx-auto max-w-[1120px] px-5 lg:px-0">
            <h2 class="text-center font-heading text-[36px] font-black text-etc-on-surface">Alur Pendaftaran Mudah</h2>
            <div class="relative mt-14 grid gap-10 md:grid-cols-5">
                ...
                @foreach ([
                    ...
                    <div class="relative z-10 text-center">
                        ...
                        <h3 class="mt-5 font-heading text-[19px] font-bold text-etc-on-surface">{{ $step['title'] }}</h3>
                        <p class="mt-3 text-[14px] leading-6 text-etc-on-muted">{{ $step['desc'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <section id="reels" class="bg-etc-charcoal py-24 text-white">
        <div class="mx-auto max-w-[1120px] px-5 lg:px-0">
            <div class="flex flex-col justify-between gap-6 md:flex-row md:items-end">
                <div>
                    <h2 class="font-heading text-[36px] font-black">Galeri Keseruan</h2>
                    <p class="mt-3 text-[16px] text-white/60">Intip suasana belajar dari reels public ETC Planet.</p>
                </div>
            ...
            @if ($reels->isNotEmpty())
                <div class="mt-12 grid gap-7 md:grid-cols-4">
                    @foreach ($reels as $reel)
                        <a href="{{ route('public.reels.show', $reel) }}" class="group overflow-hidden rounded-2xl border border-white/10 bg-etc-charcoal text-left shadow-[0_18px_34px_rgba(0,0,0,0.28)]">
                            ...
                            <div class="min-h-[118px] border-t border-white/10 p-5">
                                ...
                                <div class="mt-4 flex items-center justify-between text-[12px] text-white/60">
            ...
            @else
                <div class="mt-12 rounded-2xl border border-white/10 bg-white/5 p-8 text-center text-white/75">
    ...
    <section class="bg-[#fff0f3] py-24">
        <div class="mx-auto max-w-[1120px] px-5 text-center lg:px-0">
            <h2 class="font-heading text-[36px] font-black text-etc-on-surface">Pengajar Profesional</h2>
            <p class="mt-4 text-[16px] text-etc-on-muted">Belajar bersama instruktur yang ditampilkan khusus di halaman team ETC Planet.</p>
                ...
                <div class="mt-16 grid gap-10 md:grid-cols-4">
                    @foreach ($instructors as $teacher)
                        <article>
                            ...
                            <h3 class="mt-7 font-heading text-[21px] font-black text-etc-on-surface">{{ $teacher->name }}</h3>
                            <p class="mt-2 font-heading text-[14px] font-bold text-etc-magenta">{{ $teacher->instructor_position }}</p>
                            <p class="mt-2 text-[13px] text-etc-on-muted">{{ $teacher->instructor_specialization }}</p>
                        </article>
                    @endforeach
                </div>
            @else
                <div class="mx-auto mt-12 max-w-xl rounded-2xl border border-dashed border-[#eeb8c9] bg-white p-8 text-etc-on-muted shadow-soft">

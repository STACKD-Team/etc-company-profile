<x-layouts.public title="FAQ">
    <section class="bg-[#fff8f8] py-20">
        <div class="mx-auto max-w-[1120px] px-5 text-center lg:px-0">
            ...
            <h1 class="mt-4 font-heading text-[42px] font-black leading-tight text-[#2a1820] md:text-[56px]">{{ $page?->title ?? 'FAQ ETC Planet' }}</h1>
            <p class="mx-auto mt-5 max-w-2xl text-[16px] leading-8 text-[#765f67]">{{ $page?->body ?? 'Pertanyaan umum seputar program, biaya, jadwal, pendaftaran, pembayaran, dan placement test.' }}</p>
        </div>
    </section>
            ...
            <div class="space-y-4">
                @foreach ($faqs as $faq)
                    <details class="group rounded-[18px] border border-[#eeb8c9] bg-[#fff8f8] p-6 shadow-soft">
                        <summary class="flex cursor-pointer list-none items-center justify-between gap-4 font-heading text-lg font-black text-[#2a1820]">
                            {{ $faq['question'] }}
                            <span class="material-symbols-outlined text-etc-magenta group-open:rotate-45">add</span>
                        </summary>
                        <p class="mt-4 leading-7 text-[#765f67]">{{ $faq['answer'] }}</p>
                    </details>
                @endforeach
            </div>

            <div class="mt-10 rounded-[22px] bg-[#2b2b2b] p-8 text-center text-white">
                <h2 class="font-heading text-2xl font-black">Masih punya pertanyaan?</h2>
                <p class="mt-3 text-zinc-300">Kirim pertanyaan melalui form kontak, atau coba chatbot public lewat endpoint yang sudah tersedia.</p>

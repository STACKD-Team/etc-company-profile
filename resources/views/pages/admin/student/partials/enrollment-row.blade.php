<tr>
    ...
    <td class="py-4 pr-4">
        @if ($item->reportCard)
            <x-ui.button :href="route('admin.report-card.show', $item->reportCard)" size="sm" outlined>Rapor</x-ui.button>
        @else
            <span class="text-xs font-bold text-etc-on-muted">Belum ada</span>
        @endif
    </td>
    <td class="py-4 pr-4">
        <x-ui.button :href="route('admin.enrollment.show', $item)" size="sm" outlined>Detail</x-ui.button>
    </td>
</tr>

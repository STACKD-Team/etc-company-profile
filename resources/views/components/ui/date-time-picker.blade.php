...
<label for="{{ $id }}" class="block space-y-2">
    ...
    <x-filament::input.wrapper :valid="blank($error)" :disabled="$disabled">
        <x-filament::input :id="$id" :name="$name" type="datetime-local" :value="$fieldValue" :placeholder="$placeholder" :required="$required" :disabled="$disabled" :readonly="$readonly" {{ $attributes }} />

...
<label for="{{ $id }}" class="block space-y-2">
    ...
    <x-filament::input.wrapper :valid="blank($error)" :disabled="$disabled">
        <textarea id="{{ $id }}" name="{{ $name }}" rows="{{ $rows }}" placeholder="{{ $placeholder }}" @required($required) @disabled($disabled) @readonly($readonly) {{ $attributes->class('fi-input block w-full resize-y border-none bg-transparent py-2 text-base text-gray-950 outline-none placeholder:text-gray-400 focus:ring-0 sm:text-sm') }}>{{ $fieldValue }}</textarea>

...
<label for="{{ $id }}" class="block space-y-2">
    ...
    <x-filament::input.wrapper :valid="blank($error)" :disabled="$disabled">
        <textarea id="{{ $id }}" name="{{ $name }}" rows="{{ $rows }}" placeholder="{{ $placeholder }}" @required($required) @disabled($disabled) @readonly($readonly) {{ $attributes->class('fi-input block w-full resize-y border-none bg-transparent py-2 font-mono text-sm text-etc-on-surface outline-none placeholder:text-etc-on-muted focus:ring-0') }}>{{ $fieldValue }}</textarea>

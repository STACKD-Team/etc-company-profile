...
<div class="space-y-2" x-data="{ enabled: {{ $state }} }">
    <div class="flex items-start justify-between gap-4">
        ...
        <x-filament::toggle id="{{ $id }}" x-on:click="enabled = ! enabled" x-bind:aria-checked="enabled.toString()" x-bind:class="enabled ? 'fi-toggle-on' : 'fi-toggle-off'" :state="$state" :disabled="$disabled" {{ $attributes }} />
    </div>

@if (\Illuminate\Support\Facades\Route::has('public.chatbot.messages.store'))
    ...
    >
        <div class="hidden w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-box border-2 border-etc-outline-variant bg-etc-surface text-etc-on-surface shadow-panel" data-chatbot-panel>
            ...
            <div class="max-h-80 space-y-3 overflow-y-auto bg-etc-surface-low px-4 py-4" data-chatbot-messages>
                <p class="max-w-[85%] rounded-box bg-etc-surface px-4 py-3 text-sm leading-[var(--etc-leading-normal)] text-etc-on-muted shadow-soft">

...
test('admin academic mecca pages render for authenticated admin', function () {
    ...
    $this->actingAs($admin)->get(route('admin.students.index', [], false))->assertOk();
    $this->actingAs($admin)->get(route('admin.instructors.index', [], false))->assertOk();
    $this->actingAs($admin)->get(route('admin.programs.index', [], false))->assertOk();
    $this->actingAs($admin)->get(route('admin.programs.create', [], false))->assertOk();
    $this->actingAs($admin)->get(route('admin.classes.index', [], false))->assertOk();
    $this->actingAs($admin)->get(route('admin.classes.create', [], false))->assertOk();
    $this->actingAs($admin)->get(route('admin.enrollments.index', [], false))->assertOk();
});

test('admin academic routes require admin access', function () {
    $student = User::factory()->create(['role' => 'student']);

    $this->get(route('admin.programs.index', [], false))->assertRedirect('/login');
